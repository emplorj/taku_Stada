document.addEventListener("DOMContentLoaded", () => {
  ...
  const requesterValueDisplay = document.querySelector(
    "#requester-display .value",
  );
  ...
  const processUnits = (text, preserveSpaces = false) => {
    ...
    processedText = processedText
      .replace(/\\\[/g, "___L_BRACKET___")
      .replace(/\\\]/g, "___R_BRACKET___");
    const regex = /\[([^\[\]]*)\]/g;
    while (processedText.match(regex)) {
      processedText = processedText.replace(
        regex,
        '<span class="unit-text">$1</span>',
      );
    }
    processedText = processedText
      .replace(/___L_BRACKET___/g, "[")
      .replace(/___R_BRACKET___/g, "]");
    processedText = processedText
      .replace(/「/g, '<span class="jp-bracket jp-bracket-left">「</span>')
      .replace(/」/g, '<span class="jp-bracket jp-bracket-right">」</span>');
  ...
  const updateAll = () => {
    ...
    if (rewardInput)
      rewardValueDisplay.innerHTML = processUnits(rewardInput.value);
    if (requesterInput)
      requesterValueDisplay.innerHTML = processUnits(requesterInput.value);

    if (logoImage)
      logoImage.style.display = bgLogoToggle.checked ? "block" : "none";
    if (stampImage)
      stampImage.style.display = bgStampToggle.checked ? "block" : "none";
  ...
  const generateAndDownloadImage = async (filename) => {
    ...
    const originalOutputClass =
      requestFormPaper.classList.contains("output-format");

    requestFormPaper.style.transform = "none";
    previewPanel.style.height = "auto";
    requestFormPaper.classList.add("output-format");
    ...
    try {
      ...
      requestFormPaper.classList.toggle("output-format", originalOutputClass);
        ...
        if (data && data.length > 0) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            try {
              ...
              alert(
                `${i + 1}番目の依頼「${titleInput.value}」の画像生成中にエラーが発生しました。処理を中断します。`,
              );
              break;
            }
          }
    ...
    [
      {
        ...
        level: "Lv: 1～2",
        content:
          "この近くに蛮族が棲みつき、こちらの様子をうかがっているようなのです。何度追い払っても、どこからかすぐに現れて困っています」「いつ襲ってくるか不安なので、拠点を見つけて退治をお願いします",
        reward: "500ガメル",
        requester: "チェルシー・ソロウ",
        bgLogo: false,
        bgStamp: false,
      },
      {
        ...
        level: "Lv: 5～6",
        content:
          "街で邪教の神官が人々をたぶらかしている。奴はこれまでに何人も生贄として殺しています。犯人を特定し、止めてください。最悪、命を奪うことになっても仕方ありませんが、もし生きたまま捕縛できたなら、報酬を上乗せいたします",
        reward: "3,000ガメル",
        requester: "ブロン・ビューエル",
        bgLogo: false,
        bgStamp: true,
  ...
  const pinDirectionRadios = document.querySelectorAll(
    'input[name="pin-direction"]',
  );
  ...
  const startDrag = (e) => {
    ...
    offsetX = clientX - pinRect.left;
    offsetY = clientY - pinRect.top;
  };
  const doDrag = (e) => {
    ...
    const clientX = e.clientX || e.touches[0].clientX;
    const clientY = e.clientY || e.touches[0].clientY;
    const previewRect = requestFormPaper.getBoundingClientRect();
    let x = clientX - previewRect.left - offsetX;
    let y = clientY - previewRect.top - offsetY;
    const scale = previewRect.width / requestFormPaper.offsetWidth;
    x /= scale;
    y /= scale;
    x = Math.max(0, Math.min(x, requestFormPaper.offsetWidth - pinImage.width));
    y = Math.max(
      0,
      Math.min(y, requestFormPaper.offsetHeight - pinImage.height),
    );
  ...
  if (pinImage) {
    ...
    pinImage.addEventListener("touchstart", startDrag, { passive: false });
  }
  ...
  if (backToPreviousPageLink) {
    backToPreviousPageLink.addEventListener("click", (event) => {
