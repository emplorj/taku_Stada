document.addEventListener("DOMContentLoaded", () => {
  const TIER_LIST_CSV_URL =
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vRZD5dgrOldHCwAQ_VVeUbR6VAcTsEs4ycwXW1jLV99KUo3LtmFQgHZtRmerAL8wcfw6BBBKLvbaxDk/pub?gid=821569272&single=true&output=csv";
  ...
  const TIER_DEFINITIONS = {
    SW: {
      ...
      2: {
        name: "A,ド便利",
        color: "linear-gradient(135deg, #ff7f7f, #e63946)",
      },
      3: { name: "B,推奨", color: "linear-gradient(135deg, #ffb703, #fb8500)" },
      4: {
        name: "C,たまに使う",
        color: "linear-gradient(135deg, #f1e05a, #A79C1D)",
      },
      5: {
        name: "D,読み物としても",
        color: "linear-gradient(135deg, #7fff7f, #52b788)",
      },
      6: {
        name: "E,場合による",
        color: "linear-gradient(135deg, #7fbfff, #457b9d)",
      },
      7: {
        name: "F,シナリオとか",
        color: "linear-gradient(135deg, #7f7fff, #480ca8)",
      },
      8: {
        name: "G,上位互換有",
        color: "linear-gradient(135deg, #bfbfbf, #8d8d8d)",
      },
    },
    DX3: {
  ...
  function formatTextWithMarkdown(text) {
    return text
      ? text
          .replace(/^! (.*$)/gim, '<p class="custom-highlight-text">$1</p>')
          .replace(/^# (.*$)/gim, "<h4>$1</h4>")
          .replace(/^## (.*$)/gim, "<h5>$1</h5>")
          .replace(/^### (.*$)/gim, "<h6>$1</h6>")
  ...
  function generateHtml(systems) {
    ...
    sortedSystemNames.forEach((systemName) => {
      ...
      } else if (systemName === "SW") {
        details.classList.add("theme-sw25");
      } else if (systemName === "サタスペ") {
        details.classList.add("theme-satasupe");
      } else if (systemName === "ネクロニカ") {
        details.classList.add("theme-necronica");
      }
      ...
      Object.keys(tiers)
        ...
        .forEach((rankOrder) => {
          ...
          if (!tierInfo) {
            console.warn(
              `システム[${systemName}]のランク[${rankOrder}]の定義が見つかりません。`
            );
            return;
          }
          ...
          booksInTier.forEach((book) => {
            ...
            const categoryHtml = book.category
              ...
              : "";
            tooltip.innerHTML = `${categoryHtml}<div class="title">${
              book.title
            }</div><div class="price">${
              book.price || ""
            }</div><div class="release-date">発売日: ${
              book.releaseDate || ""
            }</div>`;
  ...
  function openBookModal(book) {
    ...
    if (book.system === "DX3") {
      modalContent.classList.add("theme-dx3");
    } else if (book.system === "SW") {
      modalContent.classList.add("theme-sw25");
    } else if (book.system === "サタスペ") {
      modalContent.classList.add("theme-satasupe");
    } else if (book.system === "ネクロニカ") {
      modalContent.classList.add("theme-necronica");
    }
    ...
    document.getElementById("book-modal-release-date").textContent =
      book.releaseDate || "N/A";
    document.getElementById("book-modal-abbreviation").textContent =
      book.abbreviation || "N/A";

    const officialInfoText = book.official_info || "（公式解説はありません）";
    document.getElementById("book-modal-official-info").innerHTML =
      formatTextWithMarkdown(officialInfoText);

    const infoText = book.info || "（個人の見解はありません）";
    document.getElementById("book-modal-info").innerHTML =
      formatTextWithMarkdown(infoText);
  ...
  async function buildAllTierLists() {
    try {
      ...
      if (!requiredHeaders.every((h) => header.includes(h))) {
        throw new Error(
          `CSVヘッダーに必須項目（${requiredHeaders.join(", ")}）がありません。`
        );
      }
