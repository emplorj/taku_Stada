...
function groupAndFormatDatesWithWeekday(fullDateStrings) {
    ...
    const formattedGroups = groups.map(group => {
        ...
        if (start.getTime() === end.getTime()) { // 単一の日付の場合
            return startStr;
        } else { // 複数日の範囲の場合
            // 終了日も常に月を含めて表示
...
async function getSharedEventData() { try { const response = await fetch(SPREADSHEET_URL); if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`); const csvText = await response.text(); const rows = parseCsvToArray(csvText); let rawEvents = []; const normalize = (str) => (typeof str !== 'string' || !str) ? '' : str.replace(/　/g, ' ').trim().replace(/\s+/g, ' '); for (let i = 1; i < rows.length; i++) { const values = rows[i]; if (!values || values.length < 4 || !normalize(values[3])) continue; const eventName = normalize(values[3]); const system = normalize(values[2]); const gm = normalize(values[4] || ''); const participants = Array.from({length: 6}, (_, j) => normalize(values[5 + j])).filter(Boolean); const dateStr = (values[1] || '').split('(')[0].trim(); const parts = dateStr.split('/'); let eventData = { id: `event-${i}`, system, eventName, gm, participants, hasDate: false }; if (parts.length === 3) { const date = new Date(parseInt(parts[0], 10), parseInt(parts[1], 10) - 1, parseInt(parts[2], 10)); if (!isNaN(date.getTime())) { eventData.date = date; eventData.endDate = new Date(date); eventData.hasDate = true; } } rawEvents.push(eventData); } const datedEvents = rawEvents.filter(e => e.hasDate).sort((a, b) => a.date.getTime() - b.date.getTime()); const mergedEvents = []; let currentGroup = null; for (const event of datedEvents) { if (currentGroup && currentGroup.system === event.system && currentGroup.eventName === event.eventName && (currentGroup.gm || '') === (event.gm || '') && areArraysEqual(currentGroup.participants, event.participants) && isNextDay(currentGroup.endDate, event.date)) { currentGroup.endDate = new Date(event.date); } else { if (currentGroup) mergedEvents.push(currentGroup); currentGroup = { ...event }; } } if (currentGroup) mergedEvents.push(currentGroup); const seriesMap = new Map(); const getSeriesKey = (e) => { const pKey = e.participants ? [...e.participants].sort().join(',') : ''; return `${(e.eventName||'').trim()}|${(e.system||'').trim()}|${(e.gm||'').trim()}|${pKey}`; }; mergedEvents.forEach(e => { const key = getSeriesKey(e); if (!seriesMap.has(key)) seriesMap.set(key, []); seriesMap.get(key).push(e); }); seriesMap.forEach(blocks => { blocks.sort((a, b) => a.date.getTime() - b.date.getTime()); const allDates = []; blocks.forEach(b => { let d = new Date(b.date); while(d.getTime() <= b.endDate.getTime()) { allDates.push(new Date(d)); d.setDate(d.getDate() + 1); }}); allDates.sort((a, b) => a.getTime() - b.getTime()); blocks.forEach(b => { b.seriesDates = allDates; b.seriesStartDate = allDates[0]; b.seriesEndDate = allDates[allDates.length - 1]; }); }); const allEvents = [...mergedEvents, ...rawEvents.filter(e => !e.hasDate)]; const eventsByDate = new Map(); mergedEvents.forEach(e => { let d = new Date(e.date); while (d.getTime() <= e.endDate.getTime()) { const dateStr = formatDate(d); if (!eventsByDate.has(dateStr)) eventsByDate.set(dateStr, []); eventsByDate.get(dateStr).push(e); d.setDate(d.getDate() + 1); } }); return { allEvents, eventsByDate }; } catch (error) { console.error("Error fetching or parsing shared event data:", error); return { allEvents: [], eventsByDate: new Map() }; } }
...
function initializeCalendar({ allEvents, eventsByDate, COLORS }) {
    ...
    if (dom.prevBtn) dom.prevBtn.addEventListener('click', () => { state.currentStartDate.setDate(state.currentStartDate.getDate() - 28); updateCalendarView(); }); 
    if (dom.nextBtn) dom.nextBtn.addEventListener('click', () => { state.currentStartDate.setDate(state.currentStartDate.getDate() + 28); updateCalendarView(); }); 
...
function initializeDaycordFeature({ allEvents, eventsByDate, COLORS }) {
    const dom = {
        ...
        tableCont: document.querySelector('#daycord-table-container .table-wrapper'),
    ...
    let daycordNames = [], schedule = [], selectedNames = [], scenarioData = [], participantPresets = {}, selectedDates = [];

    const updateDisplay = () => {
        renderTable();
        calculateAvailableDates();
        updateSelectedDatesDisplay();
        if (dom.clearBtn) dom.clearBtn.style.display = selectedNames.length > 0 ? '' : 'none';
    };
    ...
    const updateSelectedDatesDisplay = () => {
        if (dom.selectedDatesDisplay) {
            ...
            dom.selectedDatesDisplay.value = formattedDates || dom.selectedDatesDisplay.placeholder; // 候補日がない場合はプレースホルダーを使用
        }
    };
    ...
    function processScenarioData() {
        ...
        const relevantEvents = allEvents.filter(e => !e.hasDate || (e.seriesEndDate && e.seriesEndDate.getTime() >= today.getTime()));
        const map = new Map();
        ...
        scenarioData = Array.from(map, ([eventName, data]) => ({ eventName, ...data })).sort((a, b) => {
    ...
    function renderTable() {
        ...
        if (selectedNames.length === 0) {
            dom.tableCont.innerHTML = '<p>参加者を選択すると、出欠表が表示されます。</p>';
            return;
        }
        ...
        schedule.forEach(d => {
            ...
            th.innerHTML = `
                <label class="date-header-label">
                    <input type="checkbox" class="date-checkbox" data-date="${dateStr}" ${selectedDates.includes(dateStr) ? 'checked' : ''}>
        ...
        dom.tableCont.querySelectorAll('.date-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                ...
                // 対象をヘッダーセル(th)のみに限定
                const headerCell = dom.tableCont.querySelector(`th[data-date-col="${date}"]`);
    ...
    function calculateAvailableDates() {
        ...
        // groupAndFormatDatesWithWeekday を使用
        dom.resYoyu.innerHTML = yoyuDates.length > 0 ? groupAndFormatDatesWithWeekday(yoyuDates) : 'まずいかも～';
        dom.resDakyo.innerHTML = dakyoDates.length > 0 ? groupAndFormatDatesWithWeekday(dakyoDates) : 'まずいかも～';
        dom.resHiru.innerHTML = hiruDates.length > 0 ? groupAndFormatDatesWithWeekday(hiruDates) : 'まずいかも～';
    }
    ...
    async function fetchAndProcessData() {
        try {
            ...
            if (dom.copySelectedDatesBtn) dom.copySelectedDatesBtn.disabled = true; // 追加
            ...
            const sortedNamesForDropdown = [...daycordNames].sort((a,b) => a.localeCompare(b, 'ja'));
            ...
            if (dom.copySelectedDatesBtn) dom.copySelectedDatesBtn.disabled = false; // 追加
        }
    }
    ...
    if (dom.scenarioSel) dom.scenarioSel.addEventListener('change', () => {
        ...
        if (!eventName) {
            ...
            if (event) {
                const participantsOnSheet = [event.gm, ...event.participants].filter(Boolean);
                const participantsOnDaycord = participantsOnSheet.map(name => getDayCodeName(name));
                selectedNames = [...new Set(participantsOnDaycord.filter(name => daycordNames.includes(name)))];
            }
        }
        updateDisplay();
    });

    if (dom.presetSel) dom.presetSel.addEventListener('change', () => {
        const presetName = dom.presetSel.value;
        if (dom.scenarioSel) dom.scenarioSel.value = '';
        if (!presetName) {
            selectedNames = [];
        } else if (presetName === '--all-active--') {
            selectedNames = daycordNames.filter((name, index) => schedule.some(day => day.availability[index] !== '－'));
        } else {
            const presetParticipants = participantPresets[presetName] || [];
            const participantsOnDaycord = presetParticipants.map(name => getDayCodeName(name));
            selectedNames = [...new Set(participantsOnDaycord.filter(name => daycordNames.includes(name)))];
        }
        updateDisplay();
    });
    ...
    if (dom.copySelectedDatesBtn) { // コピーボタンのイベントリスナー
